...
router.post('/users/ricetta', async (req, res) => {
  ...
  try {
    ...
    const certificatePath = path.resolve(__dirname, '..', 'certs', 'user1-cert.pem');
    const privateKeyPath = path.resolve(__dirname, '..', 'certs', 'user1-key.pem');
    ...
    await gateway.connect(ccp, {
      ...
      discovery: { enabled: false, asLocalhost: true }, // Usa `asLocalhost` per sviluppo locale
    });

    console.log('Gateway connesso con successo.');

    // Ottieni il network e il chaincode
    const network = await gateway.getNetwork('medicopaziente');
    console.log('Canale medicopaziente ottenuto.');
    const contract = network.getContract('savePrescription');
    ...
    await contract.submitTransaction('SaveTransaction', ipfsHash);
